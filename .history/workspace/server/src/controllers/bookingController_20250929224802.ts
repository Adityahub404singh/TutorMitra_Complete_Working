...
// Create Booking controller function
export const createBooking = async (req: Request, res: Response) => {
  try {
    ...
    const amount = course?.price ?? tutor.feePerHour ?? 500;
...
// Get booking info between current student and the tutor
export const getMyBookingWithTutor = async (req: Request, res: Response) => {
  try {
    const user = req.user as { id?: string } | undefined;
    ...
    return res.json({
      data: {
        ...
        privateDetailsUnlocked: booking.privateDetailsUnlocked,
      },
    });
  } catch (error) {
    console.error("getMyBookingWithTutor Error:", error);
