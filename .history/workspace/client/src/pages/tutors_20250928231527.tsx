...
function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex items-center text-yellow-400">
      {[...Array(5)].map((_, i) => (
        <svg key={i} className="w-5 h-5" fill={i < Math.round(rating) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.5" viewBox="0 0 20 20">
...
function TutorAvatar({ src, alt }: { src: string; alt: string }) {
  return (
    <div className="flex items-center justify-center w-36 h-36 mb-6 bg-indigo-100 rounded-[36%] shadow-lg overflow-hidden border-4 border-white relative">
      <img src={src} alt={alt} className="object-cover w-full h-full" style={{ borderRadius: "36%" }} />
    </div>
  );
}
...
export default function Tutors() {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-gradient-to-tr from-indigo-50 to-white p-6 max-w-7xl mx-auto flex flex-col items-center">
      <h1 className="text-4xl font-extrabold text-indigo-900 my-10 text-center">Find Your Perfect Tutor</h1>
      ...
      {paginatedTutors.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-16 w-full">
          {paginatedTutors.map((tutor) => (
            <article key={tutor._id} className="bg-white rounded-2xl shadow-2xl p-12 flex flex-col items-center text-center hover:scale-105 transition-transform duration-300">
              ...
              <div className="flex flex-wrap justify-center gap-2 mb-4 text-sm">
                {tutor.subjects.map((subject) => (
                  <span key={subject} className="bg-blue-100 text-blue-700 rounded-full px-3 py-1 text-xs sm:text-sm">{subject}</span>
                ))}
              </div>
              ...
              <div className="flex items-center justify-center mt-4 text-base">
                <span className={`w-3 h-3 rounded-full mr-3 ${isTutorAvailable(tutor) ? "bg-green-600" : "bg-gray-400"}`}></span>
                <span className={isTutorAvailable(tutor) ? "text-green-800" : "text-gray-500"}>
              ...
              <div className="mt-6 flex flex-col items-center w-full gap-4">
                ...
                <button
                  ...
                  className={`w-full py-3 px-6 rounded-xl font-semibold transition-transform duration-200 ease-in-out ${isTutorAvailable(tutor) ? "bg-yellow-600 text-white hover:bg-yellow-700" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
                >
                  Book Now
                </button>
                ...
                <button
                  ...
                  className={`w-full py-3 px-6 rounded-xl font-semibold transition-transform duration-200 ease-in-out ${isTutorAvailable(tutor) ? "bg-blue-800 text-white hover:bg-blue-900" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
                >
                  Book 3-Day Trial
                </button>
              </div>
      ...
      <div className="flex justify-center items-center gap-6 mt-14 mb-8">
        <button
          ...
          disabled={page === 1}
          className={`text-lg font-semibold px-6 py-3 rounded-lg ${page === 1 ? "bg-gray-300 cursor-not-allowed" : "bg-indigo-700 text-white hover:bg-indigo-800"}`}
        >
          Previous
        </button>
        ...
        <button
          ...
          className={`text-lg font-semibold px-6 py-3 rounded-lg ${page === totalPages ? "bg-gray-300 cursor-not-allowed" : "bg-indigo-700 text-white hover:bg-indigo-800"}`}
        >
          Next
        </button>
      </div>
