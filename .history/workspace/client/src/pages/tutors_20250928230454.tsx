...
function getTutorAvatar(profileImage?: string, name: string = "Tutor") {
  if (!profileImage)
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=E0E7FF&color=3730A3&rounded=true&size=128`;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-16 w-full">
        {paginatedTutors.length === 0 && !loading ? (
          ...
          paginatedTutors.map((tutor) => (
            <article
              key={tutor._id}
              className="bg-white rounded-2xl shadow-2xl p-10 flex flex-col items-center text-center hover:scale-[1.04] transition-transform duration-400"
            >
              <img
                src={getTutorAvatar(tutor.profileImage, tutor.name)}
                alt={tutor.name}
                className="w-32 h-32 mb-5 object-cover border-4 border-indigo-200 shadow-lg transition-transform duration-300 hover:scale-110"
                style={{
                  clipPath:
              ...
              <div className="font-semibold text-xl mb-2">{tutor.name}</div>
