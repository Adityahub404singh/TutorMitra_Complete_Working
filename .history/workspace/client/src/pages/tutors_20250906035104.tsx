...
// FIXED: Always load avatars from backend's :3000 port!
function getTutorAvatar(profileImage?: string) {
  if (!profileImage) {
    // SVG fallback avatar
    return "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNDAiIGhlaWdodD0iNDAiIHZpZXdCb3g9IjAgMCA0MCA0MCIgZmlsbD0ibm9uZSIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj4KPGNpcmNsZSBjeD0iMjAiIGN5PSIyMCIgcj0iMjAiIGZpbGw9IiNGM0Y0RjYiLz48L3N2Zz4=";
  }
        ...
        {(!loading && !error && filteredTutors.length === 0) ? (
          ...
          filteredTutors.map((tutor) => {
            ...
            return (
              <article
                key={tutor._id}
                className="bg-white rounded-lg shadow-md p-6 flex flex-col items-center text-center hover:shadow-lg cursor-pointer"
                tabIndex={0}
                aria-label={`Tutor ${tutor.name}, subjects: ${tutor.subjects?.join(", ")}, located in ${tutor.city}, rating ${tutor.rating?.toFixed(1)} stars, price ${tutor.feePerHour} rupees per hour`}
              ...
              >
                <img
                  ...
                  className="w-28 h-28 rounded-full mb-5 object-cover shadow border"
                  loading="lazy"
                />
                ...
                <div className="flex items-center justify-center gap-3 mb-3">
                  ...
                  <span className={`w-3 h-3 rounded-full ${availableNow ? "bg-green-500" : "bg-gray-400"}`} aria-label={availableNow ? "Available" : "Unavailable"} />
                  <span className={`text-sm ${availableNow ? "text-green-700" : "text-gray-500"}`}>
                    {availableNow ? "Available" : "Unavailable"}
                  </span>
                </div>
                <div aria-label={`Tutor rating: ${tutor.rating} out of 5 stars`} className="flex items-center mb-3">
                  <StarRating rating={tutor.rating} />
                  <span className="ml-2 font-bold text-yellow-600">{tutor.rating?.toFixed(1)}</span>
                </div>
                <p className="font-semibold text-lg mb-4">₹{tutor.feePerHour} / hour</p>
                ...
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    if (availableNow) navigate(`/tutors/${tutor._id}`);
                  }}
