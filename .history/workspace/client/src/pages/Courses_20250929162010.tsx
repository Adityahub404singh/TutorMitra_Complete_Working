...
export default function MotivationalCoursesPage() {
  ...
  const sliderCourses = courses.slice(0, PAGE_SIZE); // First page items for slider
  const extraCourses = courses.slice(PAGE_SIZE); // Extra loaded items below slider
      ...
      <main className="flex-grow max-w-7xl mx-auto">
        ...
        <Swiper
          breakpoints={{
            ...
            768: { slidesPerView: 3, spaceBetween: 24 },  // Show 3 slides on mid screens
            1024: { slidesPerView: 6, spaceBetween: 24 }, // Show 6 on larger screens after slide
          }}
          navigation
            ...
            {extraCourses.map((course) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  <span
                    className={`uppercase text-xs font-bold px-3 py-1 rounded-full ${BADGE_COLORS[course.coachingType]} select-none`}
                    style={{ letterSpacing: "2px" }}
                  >
                    {course.coachingType}
                  </span>
                  {course.topics?.slice(0, 2).map((topic: string, i: number) => (
                    <span key={i} className="bg-blue-50 text-blue-800 text-xs rounded px-2 py-1 shadow">
                      {topic}
                    </span>
                  ))}
                </div>
