...
export default function Courses() {
  ...
  const coursesForSwiper = filteredCourses.slice(0, 6);
  const coursesForMore = filteredCourses.slice(6, 6 + (page - 1) * COURSES_PER_PAGE);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-white p-10 max-w-7xl mx-auto flex flex-col items-center">
      ...
      {showMoreList && (
        <section className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mb-10">
      ...
      {!showMoreList && filteredCourses.length > 6 && (
        <button
      ...
      {showMoreList && coursesForMore.length + 6 < filteredCourses.length && (
        <button
