    ...
    <article className="bg-white rounded-xl shadow-lg p-6 flex flex-col items-center text-center transition-all transform hover:scale-105 hover:shadow-2xl duration-300 max-w-xs mx-auto">
      ...
      <button
        ...
        className={`w-full py-2 rounded-lg text-white font-semibold transition-colors duration-300 ${
          available ? "bg-yellow-600 hover:bg-yellow-700" : "bg-gray-300 cursor-not-allowed"
        }`}
      >
        Book Now
      </button>
...
export default function Tutors() {
  ...
  const filteredTutors = tutors.filter((tutor) => {
    const subjectMatch =
      ...
      subjectQuery === "all" ||
      (tutor.subjects &&
        tutor.subjects.some((s) => s.toLowerCase().includes(subjectQuery)));
    const locationMatch =
      !locationQuery || (tutor.city || "").toLowerCase().includes(locationQuery);
    ...
    <div className="min-h-[calc(100vh-80px)] bg-gradient-to-br from-indigo-50 via-blue-50 to-white p-8 max-w-7xl mx-auto flex flex-col items-center">
      <header className="mb-10 text-center max-w-2xl">
        ...
        <p className="text-indigo-700 mb-6">
          Explore expert tutors from your city with easy booking & trial options.
        </p>
      </header>
      ...
      <section className="mt-20 bg-indigo-900 text-white rounded-3xl p-12 max-w-5xl shadow-lg text-center select-none">
        ...
        <p className="mb-6 text-lg max-w-xl mx-auto leading-relaxed">
          TutorMitra connects you with verified local tutors offering affordable,
          high-quality offline coaching in your city.
        </p>
        <ul className="list-disc list-inside max-w-md mx-auto text-left space-y-3">
          <li>Experienced tutors with proven track records.</li>
          <li>Transparent pricing with no hidden fees.</li>
          <li>Easy booking and trial session options.</li>
          <li>Personalized learning plans and support.</li>
          <li>Community trust and reliable feedback mechanisms.</li>
        </ul>
      </section>
    </div>
  );
