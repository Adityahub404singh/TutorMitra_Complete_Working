...
export default function Courses() {
  ...
  function handleBookNow(course: Course) {
    ...
    if (user.role !== "student") {
      alert("Only students can book sessions. Please login as student.");
      return;
    }
  ...
  function handleTrialBook(course: Course) {
    ...
    if (user.role !== "student") {
      alert("Only students can book trial sessions. Please login as student.");
      return;
    }
    if (!course.instructor || !course.instructor._id) {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        {/* Header with cool gradient text */}
        ...
        {/* Loading and error */}
        {loading && (
          <p className="text-primary font-semibold text-center animate-pulse">Loading courses...</p>
        )}
        {error && <p className="text-red-600 text-center font-semibold">{error}</p>}

        {/* Swiper showing courses in slides */}
        <Swiper
          slidesPerView={"auto"}
          spaceBetween={20}
          navigation
          ...
          {courses.map((course) => (
            <SwiperSlide key={course._id} className="w-[320px] flex justify-center">
              <div
                ...
                style={{
                  background: "linear-gradient(135deg, #fffbe2 70%, #e4f3ff 110%)",
                  boxShadow: "0px 16px 40px -16px rgba(37,99,235,.09), 0 1.5px 1.5px rgba(0,0,0,.03)",
                }}
              >
                ...
                <div className="relative h-40 w-full rounded-xl flex items-center justify-center mb-5 overflow-hidden shadow-lg bg-gradient-to-br from-sky-100 via-teal-100 to-white">
                  <img
                    ...
                    style={{ boxShadow: "0 6px 32px -4px rgba(100,100,130,0.14)" }}
                  />
                ...
                <div className="relative flex items-center mb-3">
                  ...
                  <div className="ml-4 flex-1">
                    ...
                    <div className="text-xs text-gray-400 flex flex-col">
                      <span className="text-[12px]">{course.instructor?.city || course.location || "No city"}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-1 text-[15px] text-gray-600 line-clamp-2">{course.description ?? ""}</div>

                {/* Badges for coachingType and topics */}
                <div className="flex flex-wrap gap-1 mb-3">
                  <span
                    className={`px-4 py-1 rounded-full text-xs font-bold uppercase shadow-sm tracking-wide ${
                      BADGE_COLORS[course.coachingType] || "bg-gray-400"
                    }`}
                ...
                {/* Booking Buttons */}
                <div className="mt-auto flex flex-col items-center gap-2 pt-3 border-t border-dashed">
                  <div className="w-full flex justify-between items-center">
                    <span className="text-xl font-extrabold text-green-600">₹{course.price}</span>
                    <button
                      onClick={() => handleBookNow(course)}
                      className="px-7 py-2 text-base rounded-full font-extrabold shadow-lg border-none"
                      style={{
                        background: "linear-gradient(90deg,#fbda61,#ff5acd)",
                        color: "#fff",
                        transition: "all .2s cubic-bezier(.7,.2,.11,1.1)",
                        boxShadow: "0 6px 40px -6px #be2afc11, 0 2px 8px 0 #fff",
                      }}
                    >
                      Book Now
                    </button>
                  ...
                  <button
                    ...
                    className="w-full mt-2 px-7 py-2 text-base rounded-full font-extrabold bg-blue-600 text-white hover:bg-blue-700 transition"
                  >
                    Book 3-Day Trial
                  </button>
                  <div className="mt-1 text-xs text-gray-500 font-semibold italic text-center">
