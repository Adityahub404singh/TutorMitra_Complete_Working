    ...
    <article className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center text-center hover:scale-105 transform transition-transform duration-300 max-w-xs mx-auto">
      ...
      <div className="flex flex-wrap justify-center gap-3 my-3">
        {tutor.subjects.map((s) => (
          <span key={s} className="bg-blue-100 text-blue-700 px-4 py-1 rounded-full text-sm font-semibold select-none">
            {s}
          </span>
        ))}
      </div>
      ...
      {/* Booking Buttons remain as before */}
      <button
        disabled={!available}
        className={`mb-3 w-full rounded-lg py-3 font-semibold text-white transition-colors duration-300 ${
      ...
      {/* View Profile Button (Optional, for extra clarity) */}
      <button
        className="w-full mt-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-2 rounded-lg"
...
export default function Tutors() {
  ...
  // Slider shows first 6 tutors full slide visible
  const tutorsForSwiper = filteredTutors.slice(0, 6);

  // More list loads next tutors beyond first 6, paginated
