...
const bookingTypes = [
  { value: "trial", label: "Trial (Low Cost, 3 days)", price: 0 },
  { value: "tutor", label: "Regular (Monthly)", price: undefined }, // Price tutor waala use karo
];
...
export default function Booking() {
  ...
  async function sendChatRequest(studentId: string, tutorId: string, message: string) {
    try {
      ...
      setChatUnlocked(true); // Chat unlock on successful request
    } catch {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Trial booking (free) case: no payment
      ...
      const options = {
        ...
        handler: async function (response: any) {
          try {
            ...
            setChatUnlocked(true); // unlock chat & contact
            navigate("/booking-confirmation", {
              state: {
                tutorName: tutor.name,
                date: sessionDate,
  ...
  const priceToShow = currentTypeObj.price !== undefined
  ...
  return (
    <section className="min-h-[80vh] flex flex-col items-center justify-center bg-transparent">
      <div className="backdrop-blur-xl bg-white/90 dark:bg-slate-900/80 rounded-2xl shadow-lg border border-blue-300 dark:border-blue-700 p-8 w-full max-w-md mx-auto">
        ...
        {/* Show unlock/ contact info & chat after booking */}
        {chatUnlocked && (
