...
function getTutorAvatar(profileImage?: string) {
  if (!profileImage) {
    return "https://ui-avatars.com/api/?name=Tutor&background=F3F4F6&color=888";
  }
...
export default function Tutors() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 max-w-7xl mx-auto">
      ...
      {loading && (
        <div className="text-center font-semibold text-indigo-700 animate-pulse py-12">Loading tutors...</div>
      )}
      {error && (
      ...
      <div className="flex justify-center items-center gap-4 mt-10">
        ...
        <span className="text-lg font-semibold">
          Page {page} of {Math.ceil(filteredTutors.length / PAGE_SIZE)}
        </span>

        <button
          onClick={() => setPage((p) => Math.min(p + 1, Math.ceil(filteredTutors.length / PAGE_SIZE)))}
          disabled={page === Math.ceil(filteredTutors.length / PAGE_SIZE)}
          className={`px-4 py-2 rounded-md ${
            page === Math.ceil(filteredTutors.length / PAGE_SIZE)
