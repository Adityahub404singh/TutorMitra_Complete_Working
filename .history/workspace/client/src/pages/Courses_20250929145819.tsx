...
export default function CoursesPage() {
  ...
  async function loadCourses(pageNum: number) {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        setError("Koi courses nahi mile.");
        setPage(-1);
      }
    } catch {
      setError("Courses lene me dikkat aayi, dobara try karo.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleBookCourse = (course: Course, trial = false) => {
    ...
    if (!user) {
      alert("Student ke roop me login karo pehle.");
      navigate("/login");
      return;
    }
    if (user.role !== "student") {
      alert("Sirf students hi book kar sakte hain.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-blue-50 font-sans px-6 py-10 flex flex-col">
      {/* Header and Motivational */}
      <header className="max-w-5xl mx-auto text-center mb-14">
        <h1 className="text-4xl font-extrabold text-blue-900 mb-3">TutorMitra - Perfect Offline Coaching</h1>
        <p className="text-lg text-gray-700 mb-2 max-w-3xl mx-auto">
          Apne liye best tutor dhoondo offline coaching ke liye — trusted profiles aur fair pricing ke saath.
        </p>
      ...
      <main className="flex-grow max-w-7xl mx-auto min-h-[60vh] pb-20">
        ...
        <Swiper
          ...
          className="pb-10"
          key={courses.length} // Important for rerender on data change
        >
          {courses.map(course => (
            <SwiperSlide key={course._id} className="flex justify-center">
              <article className="bg-white shadow-lg border border-gray-200 rounded-xl p-5 max-w-xs w-full flex flex-col">
                ...
                <div className="flex items-center mb-3">
                  ...
                  />
                  <div>
                    <h3 className="text-lg font-semibold text-blue-800 truncate">{course.instructor?.name ?? "Unknown Tutor"}</h3>
                ...
                <p className="mb-3 text-gray-700 text-sm line-clamp-3">{course.description ?? "No description available."}</p>

                <div className="flex flex-wrap gap-2 mb-3">
                  <span className={`px-2 py-1 rounded-full text-xs font-bold select-none uppercase ${BADGE_COLORS[course.coachingType]}`}>
                    {course.coachingType}
                  </span>
                  {course.topics?.slice(0, 3).map((topic, idx) => (
                    <span key={idx} className="bg-blue-200 text-blue-800 text-xs rounded px-2 truncate">{topic}</span>
                  ))}
                </div>
      ...
      {/* Footer Motivational */}
      <footer className="max-w-3xl mt-16 mx-auto p-6 bg-indigo-50 rounded-lg shadow text-center">
