...
const TutorCard = ({ tutor }: { tutor: Tutor }) => {
  ...
  return (
    <article className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center text-center max-w-xs mx-auto hover:scale-105 transform transition-transform duration-300">
      <div className="relative mb-6">
        <Avatar profileImage={tutor.profileImage} name={tutor.name} />
      </div>
      <h3 className="text-2xl font-bold text-indigo-900">{tutor.name}</h3>
      <p className="text-indigo-700 my-1">{tutor.city}</p>
      <div className="flex flex-wrap justify-center gap-3 my-3">
        {tutor.subjects.map((s) => (
          <span
            key={s}
            className="bg-blue-100 text-blue-700 px-4 py-1 rounded-full text-sm font-semibold select-none"
          >
            {s}
          </span>
        ))}
      ...
      <button
        disabled={!available}
        className={`mb-3 w-full rounded-lg py-3 font-semibold transition-colors duration-300 ${
          available ? "bg-yellow-500 text-white hover:bg-yellow-600" : "bg-gray-300 text-gray-600 cursor-not-allowed"
        }`}
      ...
      <button
        ...
        className={`w-full rounded-lg py-3 font-semibold ${
          available ? "bg-indigo-700 text-white hover:bg-indigo-800" : "bg-gray-300 text-gray-600 cursor-not-allowed"
        }`}
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-white p-10 max-w-7xl mx-auto flex flex-col items-center">
      ...
      >
        {tutorsForSwiper.map((tutor) => (
          <SwiperSlide key={tutor._id} className="flex justify-center w-[360px]">
      ...
      {!showMoreList && filteredTutors.length > 6 && (
        <button
          className="mt-10 px-10 py-4 rounded-2xl bg-indigo-700 hover:bg-indigo-800 text-white font-bold text-xl shadow-xl transition-colors"
      ...
      {showMoreList && (
        <>
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 mt-10">
          ...
          {6 + page * TUTORS_PER_PAGE < filteredTutors.length && (
            <button
              className="mt-10 px-8 py-3 rounded-2xl bg-indigo-700 hover:bg-indigo-800 text-white font-semibold shadow-lg transition-colors"
      ...
      {/* Footer Section */}
      <section className="w-full max-w-5xl p-12 mt-20 rounded-3xl bg-indigo-900 shadow-lg text-white select-none text-center">
        <h2 className="text-4xl font-extrabold mb-6">Why TutorMitra is the Best Choice</h2>
        <p className="text-lg max-w-3xl mx-auto mb-8">
          TutorMitra connects you with trusted, verified, and experienced local tutors dedicated to delivering high-quality offline coaching in your city. Transparent pricing and straightforward booking let you focus entirely on learning.
        </p>
        <ul className="max-w-3xl mx-auto text-left list-disc list-inside space-y-3 text-lg">
          ...
          <li><strong>Easy & Flexible Booking:</strong> Book sessions online and short trial classes effortlessly.</li>
          <li><strong>Personalized Learning:</strong> Tutors tailor teaching to your unique needs.</li>
          <li><strong>Community Trust:</strong> Real ratings and reviews from your city’s students.</li>
        </ul>
      </section>
    </div>
  );
