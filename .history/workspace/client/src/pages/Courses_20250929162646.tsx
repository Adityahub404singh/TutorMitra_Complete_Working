...
export default function MotivationalCoursesPage() {
  ...
  const handleBooking = (course: any, trial = false) => {
    ...
    navigate(
      `/tutors/${course.instructor._id}?courseId=${course._id}&type=${
        trial ? "trial" : "regular"
      }`
    );
  };

  // Slider me pehle 6 courses dikhao max
  // Agar total courses 6 se kam hain to jitne hain utne hi
  const sliderCourses = courses.slice(0, 6);
  // Baaki course grid me dikhao (jo 6 ke baad hain)
  const extraCourses = courses.slice(6);
      ...
      <main className="flex-grow max-w-7xl mx-auto">
        ...
        <Swiper
          breakpoints={{
            ...
            1024: { slidesPerView: 6, spaceBetween: 24 }, // 6 slides large screens
          }}
          navigation
