...
interface Review {
  _id: string;
  studentId: { _id: string; name: string; profileImage?: string };
  rating: number;
  comment: string;
  createdAt: string;
}
    ...
    <span className="flex items-center gap-0.5 text-yellow-400">
      ...
      {hasHalfStar && (
        <svg className="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
          <defs>
            <linearGradient id="half-grad"><stop offset="50%" stopColor="currentColor" /><stop offset="50%" stopColor="#fff" /></linearGradient>
          </defs>
...
function AboutPage() {
  ...
  return (
    <div className="min-h-[30vh] bg-gray-50 p-10 flex flex-col items-center">
      <header className="max-w-5xl text-center mb-16">
        ...
        <p className="text-xl text-gray-700 max-w-4xl mx-auto">
          TutorMitra ek trusted offline coaching platform hai jahan students apne shehar ke verified tutors se directly connect kar sakte hain.<br/>
  ...
  return (
    <main className="max-w-7xl mx-auto p-4 md:p-8 pb-10 space-y-10 bg-gradient-to-t from-cyan-50 via-blue-50 to-white min-h-screen">
      ...
      <section className="flex flex-wrap gap-3 justify-between items-center bg-gradient-to-r from-blue-200 to-cyan-200 rounded-lg shadow-lg p-4 mb-8 border border-cyan-300">
        <input type="text" className="px-4 py-2 rounded border border-cyan-300 shadow-sm min-w-[180px]"
          placeholder="Search by tutor or subject..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
        <select className="px-3 py-2 rounded border border-cyan-300 shadow-sm"
          value={subject} onChange={e => { setSubject(e.target.value); setPage(1); }}>
          <option value="">All Subjects</option>
          {allSubjects.map((s) => (<option key={s} value={s}>{s}</option>))}
        </select>
        <select className="px-3 py-2 rounded border border-cyan-300 shadow-sm"
          value={sort} onChange={e => setSort(e.target.value as "recent" | "name" | "distance")}>
      ...
      {loading ? (
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {displayedTutors.map((tutor) => (
            ...
            >
              {/* Profile Image always first (if profileImage given hai to show, else initials avatar) */}
              ...
              <div className="text-xs text-cyan-900 mb-4">{formatDate(tutor.createdAt)}</div>
              <button
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-4 mt-6">
          <button className="px-3 py-1 rounded bg-cyan-300 text-cyan-900 disabled:opacity-50"
            disabled={page === 1} onClick={() => setPage(page - 1)} aria-label="Previous page">Prev</button>
          <span className="font-semibold text-lg">{page} / {totalPages}</span>
          <button className="px-3 py-1 rounded bg-cyan-300 text-cyan-900 disabled:opacity-50"
            disabled={page === totalPages} onClick={() => setPage(page + 1)} aria-label="Next page">Next</button>
        </div>
      )}
      ...
      <section className="max-w-3xl mx-auto mt-12 p-6 rounded-xl shadow-lg border border-cyan-200 bg-white">
        ...
        {reviewLoading ? (
          ...
          reviews.slice(0, 4).map(({ _id, studentId, rating, comment, createdAt }) => (
            <article key={_id} className="border-b border-cyan-200 last:border-none py-2 flex items-start gap-4">
              {/* Student photo/avatar */}
              <img src={studentId.profileImage || getInitialsAvatar(studentId.name, 36)} alt={studentId.name}
                className="w-9 h-9 rounded-full object-cover border border-cyan-400 mt-1" />
              <div className="">
