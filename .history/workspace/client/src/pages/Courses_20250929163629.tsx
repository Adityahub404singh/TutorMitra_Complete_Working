...
export default function MotivationalCoursesPage() {
  ...
  // Show only first 3 courses in slider initially
  const sliderCourses = courses.slice(0, 3);
  // Max 6 courses allowed in total slider including below logic for limiting total 6
  const remainingCourses = courses.slice(3, PAGE_SIZE);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-cyan-100 via-indigo-50 to-blue-200 px-4 py-7">
      <section className="max-w-5xl mx-auto text-center mb-12">
        ...
        <p className="text-xl font-semibold text-gray-700 mb-3">“Education is the key to unlock the golden door of freedom.”</p>
      ...
      <main className="flex-grow max-w-7xl mx-auto">
        ...
        {error && (
          <p className="mb-6 text-center font-semibold text-red-500">{error}</p>
        )}
        <Swiper
            ...
            <SwiperSlide key={course._id}>
              ...
              >
                ...
                <div className="mb-4 flex gap-2">
                  <span className={`uppercase rounded-full px-4 py-1 text-xs font-bold text-white ${BADGE_COLORS[course.coachingType]}`}>
                    {course.coachingType}
                  </span>
        ...
        {/* Extra courses below slider only if page size is more than 6 */}
        {courses.length > PAGE_SIZE && (
          <section className="max-w-7xl mx-auto mt-8 grid grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {courses.slice(PAGE_SIZE).map((course) => (
              <article
                key={course._id}
                onClick={() => navigate(`/courses/${course._id}`)}
      ...
      <section className="max-w-2xl mx-auto mt-16 p-9 rounded-3xl bg-gradient-to-r from-indigo-50 via-teal-100 to-blue-50 shadow-lg text-center animate-fadeIn">
