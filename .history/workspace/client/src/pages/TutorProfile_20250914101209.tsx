...
const { user } = useAuth(); // function ke start me yeh line add karo
 ...
 // ========================= STEP 3: Add new useEffect to fetch unlock status ==========
  useEffect(() => {
    if (!id) return;
    const token = localStorage.getItem("tm_token"); // YOUR TOKEN NAME HERE
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-50 to-red-100">
        <div className="text-center p-12 bg-white rounded-3xl shadow-2xl border border-red-200">
          ...
          <div className="space-x-4">
            <button 
              onClick={() => navigate(-1)} 
              className="px-8 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold rounded-full shadow-lg hover:shadow-xl transform hover:scale-105 transition-all"
            >
              ← Go Back
            </button>
            <button 
              onClick={() => window.location.reload()} 
              className="px-8 py-3 bg-gradient-to-r from-green-600 to-teal-600 text-white font-bold rounded-full shadow-lg hover:shadow-xl transform hover:scale-105 transition-all"
            >
              Try Again
            </button>
          </div>
          ...
          <div className="relative z-10 flex flex-col lg:flex-row items-center gap-8">
            <div className="relative">
              <img
                ...
                alt="Profile"
                className="w-32 h-32 lg:w-40 lg:h-40 rounded-full border-4 border-white shadow-2xl object-cover bg-white"
                loading="lazy"
              />
...
<button
  // Button disabled jab tak user load na ho
  ...
  className={`w-full py-3 mt-4 ${(!user || !tutor.userId?._id)
    ...
    : "bg-green-600 text-white hover:bg-green-700"} rounded-xl shadow-lg transition`}
>
  💬 Start Chat
</button>
    ...
    <div>
      ...
      >
        📚 Book Your Session - ₹{tutor.feePerHour || 500}/hr
      </button>
      <button
        disabled
