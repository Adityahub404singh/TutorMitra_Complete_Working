    ...
    <div className="min-h-[30vh] bg-gray-50 p-10 flex flex-col items-center">
      ...
      <section className="max-w-7xl w-full grid grid-cols-1 sm:grid-cols-3 gap-14">
        ...
        <Feature
          ...
          description="Har tutor ki thorough verification ke baad platform par hona zaroori hai. Aap payenge genuine aur reliable teachers."
        />
        <Feature
          icon="🎯"
...
function getTutorPhoto(profileImage?: string, name?: string, size = 80) {
  const API_BASE_URL = "http://localhost:3000"; // Update this as per your API base URL if needed
  if (profileImage) {
  ...
  if (!name) return `https://ui-avatars.com/api/?name=TM&background=4fd1c5&color=fff&size=${size}`;
  const words = name.trim().split(" ");
  const initials =
    (words[0]?.[0] || "") + (words[1]?.[0] || words[0][1] || "");
  return `https://ui-avatars.com/api/?name=${initials.toUpperCase()}&background=0099E6&color=fff&size=${size}`;
          ...
          {displayedTutors.map((tutor) => (
            ...
            >
              <div className="flex gap-4 items-center mb-2">
                <img
                  src={getTutorPhoto(tutor.userId?.profileImage, tutor.userId?.name, 80)}
                  alt={`Profile of ${tutor.userId?.name || "Tutor"}`}
                  className="rounded-full w-20 h-20 object-cover border border-cyan-300"
                />
                <div>
                  <div className="text-xl font-semibold capitalize">
                    {tutor.userId?.name || "Unknown Name"}
                  </div>
